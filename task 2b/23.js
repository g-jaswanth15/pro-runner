...
//drawing roof,floor,obstacles,player and score
...
function increment(){
    ...
    //for writing score on canavs

    if(score/10>highscore || highscore==null){
        bestscore=Math.floor(score/10)
    }
...
//for changing the room after completion of one room
function something(){
    rd = Math.random()
    if(rd<0.5){
        some=  Math.floor(rd*(canvas.width-150) + 100)
...
function stoping(){
    cancelAnimationFrame(animateid) //canceling the animation of game
        gameover.play()
        jump.pause()
        power.pause()
        run.pause()
        ...
        var totalscore = Math.round(score /10) ;//score
...
//entering other room after completing one room
...
function speed(){
    ...
    if(count%4 == 0){
        ...
        var pox = enemy[0].x+140
        }
        else {
            pox = roofholes[0].x+140
        }
        powerups.x = pox
